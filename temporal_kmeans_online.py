...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for group in range(n_groups):
        ...
        if args.model.startswith('convnext'):
            model.module.classifier[-1] = torch.nn.Linear(1000, args.n_out)
        train(train_loader, model, criterion, optimizer, group, args)
...
def train(train_loader, model, criterion, optimizer, group, args):
    ...
    for epoch in range(args.epochs):
        rep = []
        print(str(group), str(epoch), 'KMeans Start')
        model.eval()
        if args.model.startswith('convnext'):
        ...
        kmeans = KMeans(n_clusters=2 * args.group_size, random_state=0).fit(rep)
        labels = kmeans.labels_
        print(str(group), str(epoch), "KMeans end")
